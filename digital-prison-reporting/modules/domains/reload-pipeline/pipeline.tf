  ...
  empty_raw_structured_and_curated_data = {
    ...
    "StepDefinition" : {
      ...
      "Parameters" : {
        "JobName" : var.glue_s3_data_deletion_job,
        "Arguments" : {
          "--dwh.file.deletion.buckets" : "${var.s3_raw_bucket_id},${var.s3_structured_bucket_id},${var.s3_curated_bucket_id}",
          "--dwh.config.key" : var.domain
        }
      },
  ...
  delete_existing_reload_diffs = {
    ...
    "StepDefinition" : {
      ...
      "Parameters" : {
        ...
        "Arguments" : {
          "--dwh.file.deletion.buckets" : var.s3_temp_reload_bucket_id,
          "--dwh.file.source.prefix" : var.reload_diff_folder,
          "--dwh.config.key" : var.domain
        }
      },
  ...
  move_reload_diffs_toInsert_to_archive_bucket = {
    ...
    "StepDefinition" : {
      ...
      "Parameters" : {
        "JobName" : var.glue_s3_file_transfer_job,
        "Arguments" : {
          "--dwh.file.transfer.source.bucket" : var.s3_temp_reload_bucket_id,
          "--dwh.file.source.prefix" : "${var.reload_diff_folder}/toInsert",
          "--dwh.file.transfer.destination.bucket" : var.s3_raw_archive_bucket_id,
          "--dwh.file.transfer.retention.period.amount" : "0",
          "--dwh.file.transfer.delete.copied.files" : "true",
          "--dwh.datastorage.retry.maxAttempts" : tostring(var.glue_s3_max_attempts),
  ...
  move_reload_diffs_toDelete_to_archive_bucket = {
    ...
    "StepDefinition" : {
      ...
      "Parameters" : {
        ...
        "Arguments" : {
          "--dwh.file.transfer.source.bucket" : var.s3_temp_reload_bucket_id,
          "--dwh.file.source.prefix" : "${var.reload_diff_folder}/toDelete",
          "--dwh.file.transfer.destination.bucket" : var.s3_raw_archive_bucket_id,
          "--dwh.file.transfer.retention.period.amount" : "0",
          "--dwh.file.transfer.delete.copied.files" : "true",
          "--dwh.datastorage.retry.maxAttempts" : tostring(var.glue_s3_max_attempts),
  ...
  move_reload_diffs_toUpdate_to_archive_bucket = {
    ...
    "StepDefinition" : {
      ...
      "Parameters" : {
        ...
        "Arguments" : {
          "--dwh.file.transfer.source.bucket" : var.s3_temp_reload_bucket_id,
          "--dwh.file.source.prefix" : "${var.reload_diff_folder}/toUpdate",
          "--dwh.file.transfer.destination.bucket" : var.s3_raw_archive_bucket_id,
          "--dwh.file.transfer.retention.period.amount" : "0",
          "--dwh.file.transfer.delete.copied.files" : "true",
          "--dwh.datastorage.retry.maxAttempts" : tostring(var.glue_s3_max_attempts),
  ...
  run_reconciliation_job = {
    ...
    "StepDefinition" : {
      ...
      "Parameters" : {
        ...
        "Arguments" : {
          "--dwh.reconciliation.checks.to.run" : "current_state_counts",
          "--dwh.config.s3.bucket" : var.s3_glue_bucket_id,
          "--dwh.config.key" : var.domain
        },
